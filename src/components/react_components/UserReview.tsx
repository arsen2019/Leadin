...
export default function UserReview() {
    ...
    return (
        <div className="mt-10 text-white p-6 rounded-lg text-center w-8/12 mx-auto min-w-[350px] pb-16">
            <h2 className="text-2xl md:text-3xl mb-3 text-semibold">You can share your opinion with others</h2>
            <form onSubmit={(e) => {e.preventDefault(); handleSubmit()}} >
                <div className="flex justify-center gap-2 mb-8">
                    {[1, 2, 3, 4, 5].map((num) => (
                        <svg width="40" height="40" viewBox="0 0 40 40" fill={``} xmlns="http://www.w3.org/2000/svg" onClick={() => handleRating(num)} key={num}>
                            <path
                                ...
                                fill={showRatingError ? "#ff8383" : num <= rating ? "#FFF84E" : "#D9D9D9"}
                            />
                        </svg>
                    ))}
                </div>
