        ...
        <div className="w-full relative">
            ...
            >
                {groupedSlides.map((group, index) => (
                    <div key={index} className="grid lg:grid-cols-2  grid-cols-1 md:gap-10 gap-5 h-fit min-h-0 mx-1">
                        {group.map((card: any) => (
                            <div key={card.id} className="cards-container flex flex-row rounded-xl  overflow-hidden ">
                                ...
                                <div
                                    className="card-content basis-3/5 flex justify-center items-center bg-[#FFF] text-[#000000] sm:p-6 p-2 w-full">
                                    <div>
            ...
            <div className="flex justify-between absolute left-0 right-0 px-4 mt-5 lg:mt-10">
                ...
                <button onClick={goToNext} disabled={currentIndex >= groupedSlides.length - itemsToShow}>
                    <img src="/icons/next.svg" alt="Next"
                         style={{filter: currentIndex >= groupedSlides.length - itemsToShow ? 'invert(0.5)' : ''}}/>
                </button>
            </div>
        </div>
    );
